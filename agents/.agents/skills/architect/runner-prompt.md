...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, rather than sharing mutable state across concurrent writers.
...
- Encode invariants in types: hard-to-misuse types beat runtime checks, which beat prose comments.
...
- Idempotent state transitions where applicable. Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy.

You are one of several runners. If you're running alongside other models or agents, produce the best design your own reasoning can make; don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
