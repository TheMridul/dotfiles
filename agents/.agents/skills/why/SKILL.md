...
Read [`epistemics.md`](epistemics.md) for the full confidence framework and phrasing guide. The synthesizer must follow it.
...
Before spawning investigators, work out which MCP servers or external tools are available to you in this environment (check your available tools/integrations list). Map each available one to one evidence category:
...
Source control is always available through git and `gh`. For the other six, classify using the tool's name, description, and the resources or actions it exposes. If a tool could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.
...
Subagent config (each): spawn a fresh, non-fork subagent (`subagent_type: general-purpose`) with normal tool access, not a read-only/plan-only mode. **Do not strip MCP/tool access.** Doing so disables MCP-backed investigators entirely. The source control investigator would be safe with read-only tools, but keep modes uniform across investigators. Investigators still shouldn't write anything to the repo; that's a posture you state in the prompt, not a sandbox restriction.

Each investigator gets:
1. The base prompt from [`investigator-prompt.md`](investigator-prompt.md)
2. The category playbook [`sources/<source>.md`](sources/) for the selected tool, adapted from the examples in [`source-playbook.md`](source-playbook.md)
3. The cross-cutting [`sources/incident-postmortem.md`](sources/incident-postmortem.md) **if the target code looks defensive** (null checks, retry logic, timeout handling, rate limiting, feature flags, egress guards, OOM handlers)
4. The code anchor from Step 2 (file paths, symbols, commit hashes, PR numbers, ticket IDs)
5. The user's original question

### Investigator roster. One per available evidence category

Spawn one investigator per category that has a matching tool available. Each owns exactly one tool.
...
Spawn one synthesizer subagent (fresh, non-fork, `subagent_type: general-purpose`, normal tool access). The synthesizer's quality check spot-verifies citations, which can require MCP/tool access, so don't strip it to a read-only mode.

The synthesizer gets:
1. The investigator findings, including any null results and any categories skipped with justification
2. The code anchor from Step 2 (file paths, symbols, commit hashes, PR numbers, ticket IDs)
3. The user's original question
4. The epistemics framework from [`epistemics.md`](epistemics.md)
5. The synthesizer prompt template from [`synthesizer-prompt.md`](synthesizer-prompt.md)
...
- Real-time team chat (Slack): skipped. No matching tool available in this environment. Gap: conversational record not searched.
...
- [`epistemics.md`](epistemics.md). Confidence tiers and phrasing guide. The synthesizer must follow it.
- [`investigator-prompt.md`](investigator-prompt.md). Base prompt template for investigator subagents.
- [`source-playbook.md`](source-playbook.md). Index pointing at the category playbooks below.
- [`sources/*.md`](sources/). One self-contained example playbook per category, plus cross-cutting `incident-postmortem.md`. Give an investigator the single file that matches its category and adapt it to the available tool.
- [`synthesizer-prompt.md`](synthesizer-prompt.md). Prompt template for the synthesizer subagent, including the output format.
